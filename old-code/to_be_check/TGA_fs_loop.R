...
## add starting value optimiser becuase these aren't working for them all.
# also redownload
...
# a <- tga[tga$species == 'Juncus_usitatus',]
# plot(tga$K, tga$deriv)

objective <- function (params, temp, obs) {

  pred <- fsTotal(temp, params)
...
paramSelect <- function (temp, obs, restarts = 300) {
...
for (i in seq_along(species_list)) {
  ...
  obs <- sp.df['deriv']
  temp <- sp.df['K']
  params_opt <- paramSelect(temp, obs, restarts = 300)

  fs <- speciesModel(sp.df, params_opt)
