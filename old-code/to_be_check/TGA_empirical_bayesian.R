...
log_likelihood <- function (data, prediction) {
  sum(dnorm(data, mean = prediction, sd = 1, log = TRUE))
}

log_prior <- function (params) {

  # HC peak location (95% probability between 240 and 270)
  lp_hc_a1 <- dnorm(params[2], 260, 15, log = TRUE)

  # CL peak location (95% probability between 300 and 330)
  lp_cl_a1 <- dnorm(params[5], 315, 15, log = TRUE)

  # LG peak location (95% probability between 340 and 400)
  lp_lg_a1 <- dnorm(params[8], 370, 20, log = TRUE)

  lp_hc_a1 + lp_cl_a1 + lp_lg_a1
...
transform_a2 <- function (theta_a2) {
  ...
  params_a2 <- theta_a2 * 0

  # second component has width (4 * sd) around 150
  params_a2[2] <- exp(theta_a2[1] / 2) * 37.5

  # add 100 on to each of the others (sd = 25)
  addition <- exp(theta_a2[c(1, 3)] / 2) * 25

  # add them
  params_a2[1] <- c(params_a2[2] + addition[1])
  params_a2[3] <- c(params_a2[2] + addition[1] + addition[2])
...
objective <- function (theta, temp, obs) {
  ...
  # see how good it is
  target <- log_likelihood(obs, pred)

  # comment this out to go frequentist
  target <- target + log_prior(params)
...
fit_peaks <- function (obs, temp, restarts = 2000) {
...
for (i in unique(tga$species)) {
  ...
  params_opt <- fit_peaks(obs, temp, restarts = 2000)
...
# # reparameterise params to make it identifiable
# params <- reparam(rnorm(9))
#
# # predict the curve
# pred <- mixture(temp, params)
#
# plot(pred ~ temp, type = 'l')
